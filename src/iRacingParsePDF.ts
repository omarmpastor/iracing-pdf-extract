import { PDFDocument, PDFItem, PDFLine, PDFPage } from 'pdf-text-extract';
...
const buildScheduleFromIndex = (pdf: any) => {
  const linesIndex = pdf.pages.flatMap((p: PDFPage) =>
    p.lines.filter((l: PDFLine) => regexIndexLine.test(l.text))
  );

  // Get first pos from all lines
  const linesStartWithTemp = linesIndex.map((l: PDFLine) => l.items[0].x);
  ...
  const disciplines: iRacingDiscipline[] = [];
  let discipline;
  let groupSerie = '';
      ...
      switch (line.items[0].x) {
        ...
        case linesStartWith[2]:
          if (discipline != undefined) {
            (<iRacingDiscipline>discipline).addSerie(new iRacingSerie(title, groupSerie));
          }
