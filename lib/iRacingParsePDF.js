...
const buildScheduleFromIndex = (pdf) => {
    let linesIndex = pdf.pages.flatMap((p) => p.lines.filter((l) => regexIndexLine.test(l.text)));
        ...
        if (titleMatch != null) {
            ...
            switch (line.items[0].x) {
                ...
                case linesStartWith[2]:
                    const serie = new iRacingSchedule_1.iRacingSerie(title, groupSerie);
                    if (discipline != undefined) {
                        discipline.addSerie(serie);
                    }
                    break;
            }
        }
...
const fillSerieFromLines = (serie, linesSerie) => {
    ...
    for (let i = 0; i < indexWeekLines.length - 1; i++) {
        ...
        const col1Week = lines[0].map((i) => i.str).join(' ').trim();
        const col2Track = lines[1].map((i) => i.str).join(' ').trim();
        const col4Duration = lines[3].map((i) => i.str).join(' ').trim();
